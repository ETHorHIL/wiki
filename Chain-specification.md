...
+ **`"params"`** contains general chain parameters:
  ...
  + `"maximumExtraDataSize"` integer specifying the maximum size in bytes of the extra_data field of the header. This must be 32 bytes or fewer as per the Ethereum Yellow Paper. It is an optional free space, some Ethrereum mining pools use it to logg their blocks. Current value for Ethereum and Kovan is "32".
