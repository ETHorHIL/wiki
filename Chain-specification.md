...
+ **`"params"`** contains general chain parameters:
  ...
  + `"chainID"` chain identifier, was introduced in EIP-155 to prevent replay attacks between the main ETH and ETC chains, which both have a networkID of 1. ChainID is an additional way to tell chains apart. Subsequent to EIP-155, ETH had a chainID of 1, while ETC had a chainID of 61 (even though they still have the same networkID of 1). networkID and chainID are  required for the chain to operate in general - e.g. it's required when signing transactions, meaning transactions signed on the ETH network end up with a different hash than those signed on ETC. Before EIP-155, signed transactions on each network looked the same, and could be replayed. See [here](https://github.com/ethereum/EIPs/blob/master/EIPS/eip-155.md) for more information.Default value is equal to `networkID`.
