...
+ **`"params"`** contains general chain parameters:
  ...
  + `"registrar"` Address of the [registrar contract](https://wiki.parity.io/Parity-name-registry.html) which enables the client to locate system contracts like autoupdating.
  + `"maxCodeSize"` - U64 - This parameter has been introduced as part of EIP170 to mitigate a vulnerability in Ethereum: when a contract is called, even though the call takes a constant amount of gas, the call can trigger O(n) cost in terms of reading the code from disk, preprocessing the code for VM execution, and also adding O(n) data to the Merkle proof for the block's proof-of-validity. Lower gas levels, this is acceptable even if suboptimal. At higher gas levels it becomes a greater concern especially for future light clients verifying proofs of validity or invalidity. Therefore a hard cap on the size of an object that can be saved to the blockchain is introduced. The cap should be set at a value slightly higher than what is feasible with current gas limits.  
